...
class TestCompress:
    ...
    def test_compression_increments_count(self, compressor):
        ...
        # increments the count even on summary failure.  Mock call_llm so
        # compress() takes the deterministic fallback path instead of
        # making a real network LLM call that hangs the test (#P14).
        with patch("agent.context_compressor.call_llm", side_effect=RuntimeError("no provider")):
    ...
    def test_protects_first_and_last(self, compressor):
        msgs = self._make_messages(10)
        # Mock call_llm so compress() uses the deterministic fallback
        # path instead of making a real network LLM call (#P14).
        with patch("agent.context_compressor.call_llm", side_effect=RuntimeError("no provider")):
            result = compressor.compress(msgs)
