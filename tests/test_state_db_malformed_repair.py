...
def test_fts_read_corruption_detected_by_read_probe(tmp_path):
    ...
    # Message varies by SQLite build (same variance documented in
    # SessionDB._is_fts_write_corruption_error and is_malformed_db_error):
    #   - older builds: "database disk image is malformed"
    #   - newer builds: 'fts5: corrupt structure record for table "..."'
    #   - 3.45.x: "vtable constructor failed: messages_fts" (shadow-table
    #     damage surfaces as a vtable constructor error rather than a
    #     malformed/disk-image message)
    # All three are the same corruption class — a damaged FTS5 shadow table
    # that makes MATCH queries fail — so accept the documented set without
    # weakening the requirement that the probe surface *some* corruption.
    assert (
...
def test_repair_rebuilds_stale_btree_indexes(tmp_path):
    ...
    assert reason is not None
    # integrity_check's wording for a stale B-tree index varies by SQLite
    # version: older builds report "wrong # of entries in index <name>";
    # 3.45.x reports "row N missing from index <name>" per missing entry.
    # Both describe the same on-disk state — an index b-tree that disagrees
    # with its base table — so accept either form while still requiring the
    # specific index name so the corruption is genuinely attributed to it.
