...
class TestSchemaInit:
    ...
    def test_reconciliation_keeps_kensei_mode_and_upstream_pin_columns(self, tmp_path):
        """A pre-v23 sessions table must acquire both additive state contracts."""
        import sqlite3

        from hermes_state import SCHEMA_SQL

        db_path = tmp_path / "state-v20.db"
        conn = sqlite3.connect(str(db_path))
        # Simulate the real v20 schema: preserve all columns/index prerequisites,
        # but omit exactly the two additive contracts this migration introduces.
        v20_schema = (
            SCHEMA_SQL.replace("    agent_mode TEXT DEFAULT 'auto',\n", "")
            .replace("    pinned INTEGER NOT NULL DEFAULT 0,\n", "")
        )
        conn.executescript(v20_schema)
        ...
        try:
            ...
            assert {"agent_mode", "pinned"}.issubset(columns)

            db.update_session_agent_mode("s1", "plan")
            assert db.set_session_pinned("s1", True) is True
            row = db._conn.execute(
                "SELECT agent_mode, pinned FROM sessions WHERE id = ?", ("s1",)
            ).fetchone()
            assert tuple(row) == ("plan", 1)
        finally:
            db.close()
