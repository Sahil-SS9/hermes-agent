...
def test_direct_session_db_flushes_share_marker_claim(agent):
    ...
    class _BarrierDB:
        ...
        def flush_token_counts(self, timeout: float = 5.0) -> bool:
            # No-op stub: _BarrierDB is a concurrency test double, not a
            # token-accounting double. _persist_session calls this on every
            # exit path (production SessionDB drains async token deltas here);
            # the test only exercises the marker/append critical section, so a
            # trivial True return preserves the contract without racing.
            return True
