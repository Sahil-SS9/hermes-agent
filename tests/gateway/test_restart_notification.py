...
@pytest.mark.asyncio
async def test_shutdown_notifications_use_configured_home_when_active_origin_missing():
    runner, adapter = make_restart_runner()
    runner.config.platforms[Platform.TELEGRAM].home_channel = HomeChannel(
        platform=Platform.TELEGRAM,
        chat_id="home-42",
...
@pytest.mark.asyncio
async def test_restart_shutdown_notification_anchors_configured_telegram_home_thread():
    runner, adapter = make_restart_runner()
    ...
    assert call.kwargs["metadata"] == {"thread_id": "20197"}
