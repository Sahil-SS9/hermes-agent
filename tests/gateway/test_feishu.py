...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_websocket_sdk_channel_ua_tag_capability_is_detected(self):
        """Adapter capability detection must match the installed SDK signature."""
        import inspect

        from lark_oapi.ws import Client as FeishuWSClient
        from plugins.platforms.feishu.adapter import FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS

        signature = inspect.signature(FeishuWSClient)
        self.assertEqual(
            FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS,
            "extra_ua_tags" in signature.parameters,
        )
    ...
    def test_connect_websocket_sets_channel_ua_tag(self):
        ...
        with (
            patch("plugins.platforms.feishu.adapter.FEISHU_AVAILABLE", True),
            patch("plugins.platforms.feishu.adapter.FEISHU_WEBSOCKET_AVAILABLE", True),
            patch("plugins.platforms.feishu.adapter.FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS", True),
    ...
    def test_connect_websocket_omits_ua_tag_when_sdk_lacks_support(self):
        """Backwards-compat fallback: if the installed lark_oapi.ws.Client
        does not accept ``extra_ua_tags`` (older 1.6.x), connect() must
        still succeed without raising TypeError and must NOT pass the
        unsupported kwarg. Group @mention routing is lost, but the channel
        stays up for P2P DM instead of crashing.
        """
        from gateway.config import PlatformConfig
        from plugins.platforms.feishu.adapter import FeishuAdapter
        ...
        with (
            patch("plugins.platforms.feishu.adapter.FEISHU_AVAILABLE", True),
            patch("plugins.platforms.feishu.adapter.FEISHU_WEBSOCKET_AVAILABLE", True),
            patch("plugins.platforms.feishu.adapter.FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS", False),
            patch("plugins.platforms.feishu.adapter.lark",
                  SimpleNamespace(LogLevel=SimpleNamespace(INFO="INFO", WARNING="WARNING"))),
            patch("plugins.platforms.feishu.adapter.EventDispatcherHandler") as mock_handler_class,
            patch("plugins.platforms.feishu.adapter.FeishuWSClient") as mock_ws_client,
        ...
        mock_ws_client.assert_called_once()
        call_kwargs = mock_ws_client.call_args.kwargs
        self.assertNotIn("extra_ua_tags", call_kwargs,
                         "must not pass extra_ua_tags to an SDK that lacks it")
