...
### LinkedIn Algorithm Tips (Sahil-specific style preferences)

First hour engagement matters most. Comments > reactions > clicks. Dwell time signals quality. External links in post body kill reach. Document posts (carousels) get strong reach.

**Sahil's preferred styles for ~30% content rotation (2026-05-15):**
- **Pawel Huryn style** — Pure tutorial/how-to setup guide, step-by-step numbered. High authority, high bookmark value.
- **Kristian Epps style** — (Full examples in memory — ask user to resend if needed)
- These styles should rotate with the standard pillars for variety. When generating LinkedIn drafts, ask the user if they want to apply one of these specific styles rather than the default pillar templates.

### Twitter/X Algorithm Tips (2026 — Based on Open-Sourced Code)

xAI open-sourced the full For You feed algorithm (May 2026, `xai-org/x-algorithm`). The following supersedes generic advice:

**Quality gate (BangerInitialScreen):** Every post runs through a Grok-based classifier before entering the ranking pipeline. It assigns:
- `quality_score` (0-1, threshold 0.4 for out-of-network distribution)
- `slop_score` (0-10, explicit AI-generation detection)
- `taxonomy_category` (content type classification)

Below 0.4 quality = no out-of-network reach. High slop_score = capped distribution.

**Ranking signals (18 weighted, from `weighted_scorer.rs`):**
- Reply chain with author response: ~150x weight (highest)
- Retweet: ~20x weight
- Bookmark / share via DM: ~10x weight
- Like: ~0.5x weight (nearly worthless for ranking)
- Negative signals: not_interested, block, mute, report (deduct from score)

**Grok multimodal embedder:** Text AND images are embedded as a combined unit. Template cards with recycled layouts score lower than real screenshots or custom visuals. Stock photos with generic text = weak multimodal signal.

**Query hydration:** The algorithm knows: mutual follow graphs (Jaccard similarity), followed topics, subscription status (Premium), impression bloom filters, demographics, IP, served history. Premium subscribers get different inference clusters.

**Taxonomy diversity requirement:** The classifier buckets each post into a taxonomy category. If your account consistently hits one bucket (e.g. always "build update"), out-of-network distribution narrows. Mix tutorial/framework posts (high bookmark value), data-driven observations (high share-via-DM), and opinion pieces (high reply rate). At least 3 taxonomy types per rolling window of N posts.

**X Premium impact:** Confirmed: new users with low engagement history get assigned to a different inference cluster. This means the algorithm explicitly treats new/Premium accounts differently.

**Implication for content strategy:** Reply game is 70% of growth. Generating original posts alone misses the single highest-leverage signal. A reply pipeline scanning target accounts for reply opportunities would cost $100/month at X API Basic tier and is deferred until budget allows. The zero-cost alternative: paste a tweet link to the agent and get 2-3 reply options generated on demand.
...
## Visual Content Generation (Hybrid Stack)

For automated social content at scale, use a **three-tier hybrid**:

1. **Free static**: Pillow data cards (prediction results, leaderboards, streak counters, stat reveals)
2. **Free motion**: FFMPEG drawtext animations (countdowns, stat mic-drops, battle teasers)
3. **Premium AI**: FAL.ai API for hero images and real-motion video (FLUX-2 Klein 9B, Wan 2.2)

Detailed architecture, pricing research, code patterns, and budget projections are in `references/near-free-visual-content-factory.md`. This was validated on an 8GB RAM VPS with a £10/month budget constraint (May 2026).

Key principle: **Always generate free first**. Use FAL.ai only for hero/launch/milestone content where photorealism or complex motion is needed.
...
- `references/x-algorithm-for-you-feed-2026.md` — Full analysis of x's open-sourced recommendation algorithm (May 2026). Covers BangerInitialScreen, slop_score, weighted scoring, taxonomy diversity, and multimodal embedder. Essential reference for any Twitter/X content strategy work.
- `references/near-free-visual-content-factory.md` — Template-based visual generation using Pillow and FFMPEG, Postiz scheduling integration, and content engine pipeline architecture.

---

## Near-Free Visual Content Factory

For high-volume social content (100+ posts/week) on minimal/zero budget, see `references/near-free-visual-content-factory.md` in this skill. Covers template-based visual generation using Pillow and FFMPEG, Postiz scheduling integration, and the content engine pipeline architecture validated during the MatchdayMaestro pre-launch content reset (May 2026).