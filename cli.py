...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin, CLIBillingMixin):
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            if width < 52:
                ...
                text = f"{battery_prefix}{model_segment} · {duration_label}"
                if goal_segment:
                    text += f" · {goal_segment}"
                if focus_label:
                    text += f" · {focus_label}"
    ...
    def _apply_model_switch_result(
        ...
        if persist_global:
            ...
            # Provider must always match the selected global model. Preserve the
            # fork's custom-endpoint safeguards: a same-provider custom model
            # selection must not erase an intentionally configured endpoint.
            ...
            _target_provider_norm = str(result.target_provider or "").strip().lower()
            _is_named_custom = _target_provider_norm.startswith("custom:")
            if _target_provider_norm != "custom" and not _is_named_custom:
                # Never persist a resolved pool credential. Clear endpoint-only
                # state when switching back to a named provider.
                save_config_value("model.api_key", None)
                save_config_value("model.api_mode", None)
            elif _is_named_custom:
                # Named custom:<name> providers sync api_mode so a global
                # switch doesn't leave the previous provider's wire-protocol
                # on disk (#25106); None clears a value the resolver didn't
                # supply. Never WRITE a resolved credential.
                save_config_value("model.api_mode", result.api_mode or None)
            elif result.api_mode:
                # Bare "custom": a resolver that explicitly supplied a custom
                # wire mode may update it; absent data leaves existing custom
                # config intact.
    ...
    def process_command(self, command: str) -> bool:
        ...
        elif canonical == "generate-image":
            self._handle_generate_image_command(cmd_original)
