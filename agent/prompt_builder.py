...
KANBAN_GUIDANCE = (
    ...
    "`kanban_request_review(summary=..., artefacts=[...], next_steps=...)` "
    "instead of `kanban_complete`. The dispatcher will spawn a reviewer "
    "profile under the `review` column; the reviewer terminates the loop "
    "with `kanban_approve` (terminal / chained / conditional), "
    "`kanban_reject`, or `kanban_reassign`. The legacy "
    "`kanban_block(reason=\"review-required: ...\")` convention still "
    "works for one release cycle but is deprecated; prefer "
    "`kanban_request_review` for all new work.\n"
