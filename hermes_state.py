...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        if fts5_available:
            ...
            # Keep legacy inline FTS untouched until the explicit v23 optimize
            # command runs. Fresh and already-optimized DBs use external-content
            # FTS. The trigram-disable policy is valid in either layout.
            trigram_disabled = os.environ.get("HERMES_DISABLE_FTS_TRIGRAM") == "1"
            expected_triggers = _BASE_FTS_TRIGGERS if trigram_disabled else _FTS_TRIGGERS
            if self._db_has_legacy_inline_fts(cursor):
                ...
                if trigram_disabled:
                    ...
                    if self._fts_enabled and triggers_need_repair:
                        self._rebuild_legacy_fts_indexes(cursor, include_trigram=False)
