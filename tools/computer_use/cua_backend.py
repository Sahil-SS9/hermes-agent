...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def type_text(self, text: str, dispatch: Optional[str] = None) -> ActionResult:
        ...
        if pid is None:
            return ActionResult(ok=False, action="type_text",
                                message="No active window — call capture() first.")
        type_args: Dict[str, Any] = {"pid": pid, "text": text}
        if dispatch:
            type_args["dispatch"] = dispatch
        return self._action("type_text", type_args)
    ...
    def key(self, keys: str, dispatch: Optional[str] = None) -> ActionResult:
        ...
        if modifiers:
            ...
            hotkey_args: Dict[str, Any] = {"pid": pid, "keys": modifiers + [key_name]}
            if dispatch:
                hotkey_args["dispatch"] = dispatch
            return self._action("hotkey", hotkey_args)
        else:
            press_args: Dict[str, Any] = {"pid": pid, "key": key_name}
            if dispatch:
                press_args["dispatch"] = dispatch
            return self._action("press_key", press_args)
