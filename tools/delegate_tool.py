...
def _build_child_agent(
    ...
    if loaded_profile_cfg:
        ...
        if _p_toolsets and (toolsets is None or not child_toolsets) and parent_toolsets:
            ...
            child_toolsets = _strip_blocked_tools(child_toolsets)
            # ``_strip_blocked_tools`` removes the ``kanban`` toolset name
            # (the candidate's kanban-isolation policy), but the profile-
            # intersection contract requires the *requested* toolset names
            # to remain observable in ``enabled_toolsets`` — the actual
            # kanban lifecycle tools are blocked via ``disabled_toolsets``
            # (``child_disabled_toolsets + ["kanban"]`` below). Re-add
            # ``kanban`` here when the profile explicitly requested it and
            # the parent exposes it, so the intersection is visible while
            # the tool schemas stay subtracted.
            if "kanban" in _p_toolsets and "kanban" in _expanded and "kanban" not in child_toolsets:
...
def delegate_task(
    ...
    def _execute_and_aggregate() -> dict:
        ...
        # Close live transcripts and update the durable manifest after every
        # result path, including synthesized and verified batches.
        for entry in results:
        ...
        if live_paths:
            response_dict["live_transcripts"] = list(live_paths)
        if synthesize and n_tasks > 1 and _get_synthesis_enabled():
...
DELEGATE_TASK_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            # ── END KENSEI CUSTOM ──
            # NOTE: acp_command/acp_args were removed from the model-facing
            # schema upstream (security hardening — the model must not be
            # able to pick an arbitrary ACP binary). Overrides now flow only
            # through trusted config (delegation credential pool), not args.
        },
        "required": [],
    },
}
