...
# Feature-detection: the ``extra_ua_tags`` Channel-signaling argument was
# added to lark_oapi.ws.Client in a later 1.6.x release.  The declared pin
# (``lark-oapi==1.6.8``) ships with it, but an environment that resolves an
# older lark-oapi (or a stale install) would raise ``TypeError`` at connect
# time.  Detect once at import; the adapter passes the tag only when the
# shipped SDK accepts it, so the group-@mention routing feature is preserved
# on supported SDKs and degrades to plain P2P on older ones instead of
# crashing.  See https://github.com/NousResearch/hermes-agent/issues/50656
FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS = False
if FeishuWSClient is not None:
    try:
        import inspect as _inspect
        FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS = "extra_ua_tags" in _inspect.signature(FeishuWSClient).parameters
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _connect_websocket(self) -> None:
        ...
        #
        # Passed only when the shipped lark_oapi.ws.Client accepts it
        # (FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS).  On an older SDK that lacks
        # the parameter, omitting it keeps connect() working (P2P DM) and
        # avoids a TypeError that would take the whole channel down.
        ws_kwargs = dict(
            app_id=self._app_id,
        ...
        if FEISHU_WS_SUPPORTS_EXTRA_UA_TAGS:
            ws_kwargs["extra_ua_tags"] = ["channel"]
        else:
            logger.warning(
                "[Feishu] lark_oapi.ws.Client lacks extra_ua_tags support; "
                "group @mention routing over WebSocket is disabled. "
                "Upgrade lark-oapi to >=1.6.8 to enable it."
            )
